...
For example, define an age-structured two-state model of good and bad health, with transitions between health states as well as from each health state to death. If transition rate schedules are held fixed for a very long time, such that strong ergodicity takes hold, and the population on the whole is of constant size, then each new cohort is expected be identical, and the numbers of healthy and sick individuals in each age are expected to be the same in each time step. 

Individual imaginary lifecourse trajectories resulting from fixed transition rate schedules consist in sequences of states. Such trajectories are not completely random processes, but these are constrained in the limit to abide by the laws of probability. Specifically, the probability of observing any particular lifecourse trajectory is the product of the sequence of transition probabilities required to produce it. 

For example, say we have a life summarized in five discrete time steps, \texttt{H} for healthy and \texttt{S} for sick:
\begin{center}
\Large
\texttt{H H H S S}\\
\texttt{0 1 2 3 4}
\end{center}
The probability of observing exactly this life is the probability of being born healthy times that of surviving to age 1 and remaining healthy, times that of surviv .... STOPPED EDITING HERE. FROM HERE DOWN SAME AS BEFORE I THINK.
